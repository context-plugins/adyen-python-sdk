"""adyen.

This file was automatically generated by APIMATIC v3.0 ( https://www.apimatic.io ).
"""

# ruff: noqa: E501
from adyen.api_helper import APIHelper


class PlatformChargebackLogic4(object):
    """Implementation of the 'PlatformChargebackLogic4' model.

    Dictates the behavior of how a potential chargeback should be booked when using
    Adyen Platforms.

    Attributes:
        behavior (BehaviorEnum): The method of handling the chargeback.  Possible
            values: **deductFromLiableAccount**, **deductFromOneBalanceAccount**,
            **deductAccordingToSplitRatio**.
        cost_allocation_account (str): The unique identifier of the balance account
            to which the chargeback fees are booked. By default, the chargeback fees
            are booked to your liable balance account.
        target_account (str): The unique identifier of the balance account against
            which the disputed amount is booked.  Required if `behavior` is
            **deductFromOneBalanceAccount**.

    """

    # Create a mapping from Model property names to API property names
    _names = {
        "behavior": "behavior",
        "cost_allocation_account": "costAllocationAccount",
        "target_account": "targetAccount",
    }

    _optionals = [
        "behavior",
        "cost_allocation_account",
        "target_account",
    ]

    def __init__(
        self,
        behavior=APIHelper.SKIP,
        cost_allocation_account=APIHelper.SKIP,
        target_account=APIHelper.SKIP):
        """Initialize a PlatformChargebackLogic4 instance."""
        # Initialize members of the class
        if behavior is not APIHelper.SKIP:
            self.behavior = behavior
        if cost_allocation_account is not APIHelper.SKIP:
            self.cost_allocation_account = cost_allocation_account
        if target_account is not APIHelper.SKIP:
            self.target_account = target_account

    @classmethod
    def from_dictionary(cls,
                        dictionary):
        """Create an instance of this model from a dictionary

        Args:
            dictionary (dictionary): A dictionary representation of the object
            as obtained from the deserialization of the server's response. The
            keys MUST match property names in the API description.

        Returns:
            object: An instance of this structure class.

        """
        if not isinstance(dictionary, dict) or dictionary is None:
            return None

        # Extract variables from the dictionary
        behavior =\
            dictionary.get("behavior")\
            if dictionary.get("behavior")\
                else APIHelper.SKIP
        cost_allocation_account =\
            dictionary.get("costAllocationAccount")\
            if dictionary.get("costAllocationAccount")\
                else APIHelper.SKIP
        target_account =\
            dictionary.get("targetAccount")\
            if dictionary.get("targetAccount")\
                else APIHelper.SKIP

        # Return an object of this model
        return cls(behavior,
                   cost_allocation_account,
                   target_account)

    def __repr__(self):
        """Return a unambiguous string representation."""
        _behavior=(
            self.behavior
            if hasattr(self, "behavior")
            else None
        )
        _cost_allocation_account=(
            self.cost_allocation_account
            if hasattr(self, "cost_allocation_account")
            else None
        )
        _target_account=(
            self.target_account
            if hasattr(self, "target_account")
            else None
        )
        return (
            f"{self.__class__.__name__}("
            f"behavior={_behavior!r}, "
            f"cost_allocation_account={_cost_allocation_account!r}, "
            f"target_account={_target_account!r}, "
            f")"
        )

    def __str__(self):
        """Return a human-readable string representation."""
        _behavior=(
            self.behavior
            if hasattr(self, "behavior")
            else None
        )
        _cost_allocation_account=(
            self.cost_allocation_account
            if hasattr(self, "cost_allocation_account")
            else None
        )
        _target_account=(
            self.target_account
            if hasattr(self, "target_account")
            else None
        )
        return (
            f"{self.__class__.__name__}("
            f"behavior={_behavior!s}, "
            f"cost_allocation_account={_cost_allocation_account!s}, "
            f"target_account={_target_account!s}, "
            f")"
        )
